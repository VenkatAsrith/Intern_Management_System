import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { Card } from "@/components/ui/card";
import {
  Dialog,
  DialogContent,
  DialogHeader,
  DialogTitle,
} from "@/components/ui/dialog";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { Skeleton } from "@/components/ui/skeleton";
import { Textarea } from "@/components/ui/textarea";
import { useAuth } from "@/hooks/use-auth";
import {
  useCreateSubmission,
  useSubmissions,
  useUpdateSubmissionStatus,
} from "@/hooks/useSubmissions";
import type { SubmissionStatus, WorkSubmission } from "@/hooks/useSubmissions";
import {
  CheckCircle,
  ExternalLink,
  FileText,
  GitBranch,
  HardDrive,
  MessageSquare,
  Plus,
  XCircle,
} from "lucide-react";
import { useState } from "react";

const STATUS_MAP: Record<
  SubmissionStatus,
  { label: string; cssClass: string; icon: React.ReactNode }
> = {
  pending: {
    label: "Pending Review",
    cssClass: "submission-status-pending",
    icon: <FileText className="w-3 h-3" />,
  },
  approved: {
    label: "Approved",
    cssClass: "submission-status-approved",
    icon: <CheckCircle className="w-3 h-3" />,
  },
  rejected: {
    label: "Rejected",
    cssClass: "submission-status-rejected",
    icon: <XCircle className="w-3 h-3" />,
  },
  feedback_given: {
    label: "Feedback Given",
    cssClass: "",
    icon: <MessageSquare className="w-3 h-3" />,
  },
};

function StatusBadge({ status }: { status: SubmissionStatus }) {
  const s = STATUS_MAP[status];
  const colorClass =
    status === "pending"
      ? "bg-blue-500/15 text-blue-400 border-blue-500/30"
      : status === "approved"
        ? "bg-emerald-500/15 text-emerald-400 border-emerald-500/30"
        : status === "rejected"
          ? "bg-red-500/15 text-red-400 border-red-500/30"
          : "bg-amber-500/15 text-amber-400 border-amber-500/30";
  return (
    <Badge
      className={`${colorClass} border text-[10px] gap-1 inline-flex items-center`}
    >
      {s.icon}
      {s.label}
    </Badge>
  );
}

function NewSubmissionModal({
  open,
  onClose,
}: { open: boolean; onClose: () => void }) {
  const create = useCreateSubmission();
  const [form, setForm] = useState({
    title: "",
    description: "",
    githubLink: "",
    driveLink: "",
    notes: "",
  });

  const handleSubmit = async () => {
    if (!form.title.trim()) return;
    await create.mutateAsync({
      title: form.title.trim(),
      description: form.description.trim(),
      githubLink: form.githubLink.trim() || undefined,
      driveLink: form.driveLink.trim() || undefined,
      fileUrls: [],
      notes: form.notes.trim(),
    });
    setForm({
      title: "",
      description: "",
      githubLink: "",
      driveLink: "",
      notes: "",
    });
    onClose();
  };

  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="bg-card border-border max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Plus className="w-4 h-4 text-primary" />
            New Submission
          </DialogTitle>
        </DialogHeader>
        <div className="space-y-4">
          <div className="space-y-1.5">
            <Label>
              Title <span className="text-primary">*</span>
            </Label>
            <Input
              value={form.title}
              onChange={(e) =>
                setForm((f) => ({ ...f, title: e.target.value }))
              }
              placeholder="What are you submitting?"
              data-ocid="submissions.title_input"
            />
          </div>
          <div className="space-y-1.5">
            <Label>Description</Label>
            <Textarea
              value={form.description}
              onChange={(e) =>
                setForm((f) => ({ ...f, description: e.target.value }))
              }
              placeholder="Describe what you built or completed…"
              rows={3}
              data-ocid="submissions.description_textarea"
            />
          </div>
          <div className="space-y-1.5">
            <Label>GitHub Link</Label>
            <div className="relative">
              <GitBranch className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-muted-foreground" />
              <Input
                value={form.githubLink}
                onChange={(e) =>
                  setForm((f) => ({ ...f, githubLink: e.target.value }))
                }
                placeholder="https://github.com/…"
                className="pl-8"
                data-ocid="submissions.github_input"
              />
            </div>
          </div>
          <div className="space-y-1.5">
            <Label>Google Drive Link</Label>
            <div className="relative">
              <HardDrive className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-muted-foreground" />
              <Input
                value={form.driveLink}
                onChange={(e) =>
                  setForm((f) => ({ ...f, driveLink: e.target.value }))
                }
                placeholder="https://drive.google.com/…"
                className="pl-8"
                data-ocid="submissions.drive_input"
              />
            </div>
          </div>
          <div className="space-y-1.5">
            <Label>Notes</Label>
            <Textarea
              value={form.notes}
              onChange={(e) =>
                setForm((f) => ({ ...f, notes: e.target.value }))
              }
              placeholder="Any additional context or notes…"
              rows={2}
              data-ocid="submissions.notes_textarea"
            />
          </div>
          <div className="flex justify-end gap-2">
            <Button
              type="button"
              variant="outline"
              onClick={onClose}
              data-ocid="submissions.cancel_button"
            >
              Cancel
            </Button>
            <Button
              type="button"
              disabled={!form.title.trim() || create.isPending}
              onClick={() => void handleSubmit()}
              data-ocid="submissions.submit_button"
            >
              Submit
            </Button>
          </div>
        </div>
      </DialogContent>
    </Dialog>
  );
}

function ReviewModal({
  submission,
  open,
  onClose,
}: {
  submission: WorkSubmission;
  open: boolean;
  onClose: () => void;
}) {
  const update = useUpdateSubmissionStatus();
  const [feedback, setFeedback] = useState("");
  const [action, setAction] = useState<
    "approved" | "rejected" | "feedback_given"
  >("approved");

  const handleSubmit = async () => {
    await update.mutateAsync({ id: submission.id, status: action, feedback });
    setFeedback("");
    onClose();
  };

  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent
        className="bg-card border-border max-w-sm"
        data-ocid="submissions.review_dialog"
      >
        <DialogHeader>
          <DialogTitle>Review Submission</DialogTitle>
        </DialogHeader>
        <div className="space-y-4">
          <p className="text-sm font-medium">{submission.title}</p>
          <p className="text-xs text-muted-foreground">
            by {submission.internName}
          </p>
          <div className="space-y-1.5">
            <Label>Decision</Label>
            <div className="flex gap-2">
              {(["approved", "rejected", "feedback_given"] as const).map(
                (s) => (
                  <button
                    key={s}
                    type="button"
                    onClick={() => setAction(s)}
                    className={`flex-1 py-1.5 text-xs rounded border transition-smooth ${
                      action === s
                        ? "border-primary bg-primary/20 text-foreground"
                        : "border-border text-muted-foreground"
                    }`}
                    data-ocid={`submissions.review_${s}_button`}
                  >
                    {STATUS_MAP[s].label}
                  </button>
                ),
              )}
            </div>
          </div>
          <div className="space-y-1.5">
            <Label>Feedback (optional)</Label>
            <Textarea
              value={feedback}
              onChange={(e) => setFeedback(e.target.value)}
              placeholder="Add feedback for the intern…"
              rows={3}
              data-ocid="submissions.feedback_textarea"
            />
          </div>
          <div className="flex justify-end gap-2">
            <Button
              type="button"
              variant="outline"
              onClick={onClose}
              data-ocid="submissions.review_cancel_button"
            >
              Cancel
            </Button>
            <Button
              type="button"
              disabled={update.isPending}
              onClick={() => void handleSubmit()}
              data-ocid="submissions.review_confirm_button"
            >
              Submit Review
            </Button>
          </div>
        </div>
      </DialogContent>
    </Dialog>
  );
}

function SubmissionCard({
  submission,
  canReview,
}: {
  submission: WorkSubmission;
  canReview: boolean;
}) {
  const [reviewOpen, setReviewOpen] = useState(false);
  const statusClass =
    submission.status === "pending"
      ? "submission-status-pending"
      : submission.status === "approved"
        ? "submission-status-approved"
        : submission.status === "rejected"
          ? "submission-status-rejected"
          : "";

  return (
    <div className={`submission-card ${statusClass}`}>
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            <p className="font-semibold text-sm truncate">{submission.title}</p>
            <StatusBadge status={submission.status} />
          </div>
          {submission.internName && (
            <p className="text-xs text-primary mt-0.5">
              {submission.internName}
            </p>
          )}
          <p className="text-xs text-muted-foreground mt-0.5">
            {new Date(submission.createdAt).toLocaleDateString("en-IN", {
              day: "2-digit",
              month: "short",
              year: "numeric",
            })}
          </p>
        </div>
        {canReview && submission.status === "pending" && (
          <Button
            type="button"
            variant="outline"
            size="sm"
            className="text-xs shrink-0"
            onClick={() => setReviewOpen(true)}
            data-ocid={`submissions.review_open_modal_button.${submission.id}`}
          >
            Review
          </Button>
        )}
      </div>

      {submission.description && (
        <p className="text-sm text-muted-foreground mt-2 line-clamp-2">
          {submission.description}
        </p>
      )}

      <div className="flex flex-wrap gap-2 mt-2">
        {submission.githubLink && (
          <a
            href={submission.githubLink}
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-1.5 text-xs text-primary hover:text-primary/80 transition-colors"
          >
            <GitBranch className="w-3 h-3" />
            GitHub
            <ExternalLink className="w-2.5 h-2.5" />
          </a>
        )}
        {submission.driveLink && (
          <a
            href={submission.driveLink}
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-1.5 text-xs text-primary hover:text-primary/80 transition-colors"
          >
            <HardDrive className="w-3 h-3" />
            Drive
            <ExternalLink className="w-2.5 h-2.5" />
          </a>
        )}
      </div>

      {submission.adminFeedback && (
        <div className="notes-comment mt-3">
          <p className="text-xs font-semibold text-muted-foreground mb-0.5">
            Admin Feedback
          </p>
          <p className="text-sm">{submission.adminFeedback}</p>
          {submission.reviewedBy && (
            <p className="text-[10px] text-muted-foreground mt-1">
              — {submission.reviewedBy}
            </p>
          )}
        </div>
      )}

      {reviewOpen && (
        <ReviewModal
          submission={submission}
          open={reviewOpen}
          onClose={() => setReviewOpen(false)}
        />
      )}
    </div>
  );
}

export function SubmissionsPage() {
  const { isAdmin, sessionToken } = useAuth();
  const admin = isAdmin();
  const myInternId =
    !admin && sessionToken ? sessionToken.split(":")[0] : undefined;

  const { data: mySubmissions, isLoading: myLoading } = useSubmissions(
    admin ? null : myInternId,
  );
  const { data: allSubmissions, isLoading: allLoading } = useSubmissions();

  const [newOpen, setNewOpen] = useState(false);
  const [statusFilter, setStatusFilter] = useState<SubmissionStatus | "all">(
    "all",
  );
  const [search, setSearch] = useState("");

  const submissions = admin ? allSubmissions : mySubmissions;
  const isLoading = admin ? allLoading : myLoading;

  const filtered =
    submissions?.filter((s) => {
      const statusMatch = statusFilter === "all" || s.status === statusFilter;
      const searchMatch =
        !search ||
        s.title.toLowerCase().includes(search.toLowerCase()) ||
        s.internName.toLowerCase().includes(search.toLowerCase());
      return statusMatch && searchMatch;
    }) ?? [];

  const STATUS_FILTERS: { label: string; value: SubmissionStatus | "all" }[] = [
    { label: "All", value: "all" },
    { label: "Pending", value: "pending" },
    { label: "Approved", value: "approved" },
    { label: "Rejected", value: "rejected" },
    { label: "Feedback Given", value: "feedback_given" },
  ];

  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold">Work Submissions</h1>
          <p className="text-sm text-muted-foreground mt-0.5">
            {admin
              ? "Review and approve intern submissions"
              : "Track your submitted work"}
          </p>
        </div>
        {!admin && (
          <Button
            type="button"
            onClick={() => setNewOpen(true)}
            data-ocid="submissions.new_submission_button"
          >
            <Plus className="w-4 h-4 mr-2" />
            New Submission
          </Button>
        )}
      </div>

      {/* Filters */}
      <Card className="bg-card border-border p-4">
        <div className="flex flex-wrap gap-3 items-end">
          <div className="space-y-1">
            <Label className="text-xs">Status</Label>
            <div className="flex gap-1.5 flex-wrap">
              {STATUS_FILTERS.map((f) => (
                <button
                  key={f.value}
                  type="button"
                  onClick={() => setStatusFilter(f.value)}
                  className={`px-3 py-1 text-xs rounded-full border transition-smooth ${
                    statusFilter === f.value
                      ? "border-primary bg-primary/20 text-foreground"
                      : "border-border text-muted-foreground hover:border-primary/50"
                  }`}
                  data-ocid={`submissions.filter.${f.value}`}
                >
                  {f.label}
                </button>
              ))}
            </div>
          </div>
          {admin && (
            <div className="space-y-1">
              <Label className="text-xs">Search</Label>
              <Input
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                placeholder="Search by title or intern…"
                className="h-8 text-sm w-52"
                data-ocid="submissions.search_input"
              />
            </div>
          )}
        </div>
      </Card>

      {/* Submissions */}
      {isLoading ? (
        <div className="space-y-4">
          {[1, 2, 3].map((i) => (
            <div key={i} className="submission-card">
              <Skeleton className="h-4 w-48 mb-2" />
              <Skeleton className="h-3 w-full" />
            </div>
          ))}
        </div>
      ) : filtered.length === 0 ? (
        <div
          className="flex flex-col items-center justify-center py-20 text-muted-foreground"
          data-ocid="submissions.empty_state"
        >
          <FileText className="w-12 h-12 mb-3 opacity-20" />
          <p className="text-sm font-medium">
            {admin ? "No submissions found" : "No submissions yet"}
          </p>
          {!admin && (
            <Button
              type="button"
              className="mt-4"
              onClick={() => setNewOpen(true)}
              data-ocid="submissions.empty_new_button"
            >
              <Plus className="w-4 h-4 mr-2" /> Submit Your First Work
            </Button>
          )}
        </div>
      ) : admin ? (
        /* Admin table-like card list */
        <div className="space-y-3">
          <p className="text-xs text-muted-foreground">
            Showing {filtered.length} submission
            {filtered.length !== 1 ? "s" : ""}
          </p>
          {filtered.map((s, idx) => (
            <div key={s.id} data-ocid={`submissions.item.${idx + 1}`}>
              <SubmissionCard submission={s} canReview={admin} />
            </div>
          ))}
        </div>
      ) : (
        /* Intern card grid */
        <div className="grid gap-4 sm:grid-cols-2">
          {filtered.map((s, idx) => (
            <div key={s.id} data-ocid={`submissions.item.${idx + 1}`}>
              <SubmissionCard submission={s} canReview={false} />
            </div>
          ))}
        </div>
      )}

      <NewSubmissionModal open={newOpen} onClose={() => setNewOpen(false)} />
    </div>
  );
}
