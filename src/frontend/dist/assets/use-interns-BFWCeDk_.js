...
import { J as Subscribable, K as shallowEqualObjects, N as hashKey, O as getDefaultState, Q as notifyManager, s as useQueryClient, r as reactExports, T as noop$1, V as shouldThrowError, c as createLucideIcon, j as jsxRuntimeExports, W as React, e as composeRefs, I as reactDomExports, d as useComposedRefs, Y as ReactDOM, R as React$1, f as cn, o as useBackend, p as useQuery, Z as convertIntern, u as useAuth, n as ue } from "./index-BIeIRmcz.js";
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const results = await actor.listInterns({
        status: (filter == null ? void 0 : filter.status) ?? void 0,
        space: (filter == null ? void 0 : filter.space) ?? void 0
      });
      const converted = results.map(convertIntern);
      if (filter == null ? void 0 : filter.search) {
        const q = filter.search.toLowerCase();
        return converted.filter(
          (i) => i.name.toLowerCase().includes(q) || i.email.toLowerCase().includes(q) || i.department.toLowerCase().includes(q) || i.phone.includes(q)
        );
      }
      return converted;
    },
...
function useCreateIntern() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      ue.success("Intern created successfully");
    },
    onError: (e) => ue.error(e.message || "Failed to create intern")
  });
}
function useUpdateIntern() {
  const { actor } = useBackend();
  const { sessionToken } = useAuth();
  const qc = useQueryClient();
  return useMutation({
    ...
    onSuccess: (_, { id }) => {
      ...
      qc.invalidateQueries({ queryKey: ["dashboard"] });
      ue.success("Intern updated successfully");
    },
    onError: (e) => ue.error(e.message || "Failed to update intern")
  });
}
function useDeleteIntern() {
  const { actor } = useBackend();
  const { sessionToken } = useAuth();
  const qc = useQueryClient();
  return useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["interns"] });
      qc.invalidateQueries({ queryKey: ["dashboard"] });
      ue.success("Intern deleted successfully");
    },
    onError: (e) => ue.error(e.message || "Failed to delete intern")
  });
}
function useUpdateDocumentState() {
  const { actor } = useBackend();
  const { sessionToken } = useAuth();
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async ({
      ...
      if (!actor || !sessionToken) throw new Error("Not connected");
      const docType = field.includes("offerLetter") ? "offerLetter" : field.includes("certificate") && !field.includes("completion") ? "certificate" : "completionLetter";
      const isSentField = field.endsWith("Sent");
      const action = isSentField ? value ? "sent" : "unsent" : value ? "opened" : "unopened";
    ...
    onError: (e) => ue.error(e.message || "Failed to update document status")
  });
}
function useLogWhatsApp() {
  const { actor } = useBackend();
  const { sessionToken } = useAuth();
  const qc = useQueryClient();
  return useMutation({
    ...
    onSuccess: (_, internId) => {
      qc.invalidateQueries({ queryKey: ["intern", internId] });
      qc.invalidateQueries({ queryKey: ["interns"] });
      ue.success("WhatsApp contact logged");
    },
    onError: (e) => ue.error(e.message || "Failed to log WhatsApp contact")
  });
}
export {
  Anchor as A,
  ...
  Check as o,
  useInterns as p,
  useDeleteIntern as q,
  useCreateIntern as r,
  useMutation as s,
  useIntern as t,
  useCallbackRef$1 as u,
  useLogWhatsApp as v,
  useUpdateDocumentState as w,
  useUpdateIntern as x,
